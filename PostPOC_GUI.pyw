...
def generate_poc(request: RequestData, filename: str):
    def generate_input_field(name: str, value: str) -> str:
        input_elem = """  <tr>
    <td>{0}</td>
    <td><input type='text' name='{0}' value='{1}' size='54'/></td>
...
</html>""".format(escape(request.full_url), input_fields)
        f.write(page_source)
